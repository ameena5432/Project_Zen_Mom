...
def cleanse_Moms_Response_data(momsResponseDF):
    momsResponseDF.dropna(subset=["survey_response_id", "mom_id", "session_id", "question_id", "response_Score"],
                          inplace=True)
...
def merge_dataframes(pumpingSessionDF, surveyQuestionsDF, momsResponseDF):
    ...
    queryAnswers = pumpingResponseSurvey[
        ['mom_id_x', 'session_id', 'Timestamp', 'Milk_Volume', 'Question_text', 'Accessed_factor', 'response_Score']]
    return queryAnswers
...
def factor_label_winner_Calculations(queryAnswers):
    ...
    values = ['low ' + grouped['Accessed_factor'].str.split('_', expand=True)[0],
              'medium ' + grouped['Accessed_factor'].str.split('_', expand=True)[0],
              'high ' + grouped['Accessed_factor'].str.split('_', expand=True)[0]]
